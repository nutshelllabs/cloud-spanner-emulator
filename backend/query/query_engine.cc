...
class VectorsRowCursor : public RowCursor {
 public:
  VectorsRowCursor(
      ...
      const std::vector<std::vector<googlesql::Value>> column_values)
...
absl::StatusOr<googlesql::AnalyzerOptions> MakeAnalyzerOptionsWithParameters(
    const googlesql::ParameterValueMap& params, const std::string time_zone) {
  googlesql::AnalyzerOptions options = MakeGoogleSqlAnalyzerOptions(time_zone);
  for (const auto& [name, value] : params) {
    GOOGLESQL_RETURN_IF_ERROR(options.AddQueryParameter(name, value.type()));
  }
...
// ideally hooking into GoogleSQL to control error messages.
absl::Status MaybeTransformGoogleSQLDMLError(absl::Status error) {
...
bool IsGenerated(const googlesql::Column* column) {
  return column->GetAs<QueryableColumn>()->wrapped_column()->is_generated();
}

bool HasDefaultValue(const googlesql::Column* column) {
  return column->GetAs<QueryableColumn>()
      ->wrapped_column()
      ->has_default_value();
}

bool IsPendingCommitTimestampColumn(const googlesql::Column* column) {
  return column->GetAs<QueryableColumn>()
      ->wrapped_column()
      ->is_pending_commit_timestamp();
}

bool IsOnUpdateColumn(const googlesql::Column* column) {
  return column->GetAs<QueryableColumn>()->wrapped_column()->has_on_update();
}

bool IsKeyColumn(const googlesql::Table* table,
...
class ReturningCommitTimestampChecker : public googlesql::ResolvedASTVisitor {
 public:
...
absl::StatusOr<std::unique_ptr<RowCursor>> BuildReturningRowResult(
  ...
  GOOGLESQL_RET_CHECK_NE(returning, nullptr);
  ...
  for (const auto& resolved_named_expr : returning->expr_list()) {
    GOOGLESQL_RETURN_IF_ERROR(resolved_named_expr->Accept(&checker));
  ...
  for (const auto& named_column : returning->output_column_list()) {
    const googlesql::ResolvedColumn& column = named_column->column();
...
absl::StatusOr<std::tuple<Mutation, int64_t, bool>> BuildInsert(
    ...
    const std::vector<googlesql::ResolvedColumn>& insert_columns,
  ...
  for (int i = 0; i < table->NumColumns(); ++i) {
    ...
    // Include default columns in the INSERT mutation as GoogleSQL has
    // already evaluated the default value so we already know the value if the
    // user did not already specify it.
    // We cannot use this logic for UPDATEs because the default values will
    // override the existing row value.
    if (HasDefaultValue(table->GetColumn(i)) &&
        op_type == MutationOpType::kInsert) {
      column_names.push_back(column_name);
      continue;
    }
  ...
  bool has_rows = false;
  std::map<std::string, googlesql::Value> fallback_defaults;
  while (iterator->NextRow()) {
    has_rows = true;
    values.emplace_back();
    Key row_key;
    for (int i = 0; i < table->NumColumns(); ++i) {
      googlesql::Value column_value = iterator->GetColumnValue(i);
      const auto& column_name = table->GetColumn(i)->Name();
      if (pending_ts_columns.find(column_name) != pending_ts_columns.end()) {
        column_value =
            googlesql::Value::StringValue(kCommitTimestampIdentifier);
      } else if (HasDefaultValue(table->GetColumn(i))) {
        // Grab the evaluated value for the default column from the first row
        if (!values.empty()) {
          fallback_defaults[table->GetColumn(i)->Name()] = column_value;
        }
      }
  ...
  mutation.AddWriteOp(op_type, table->FullName(), column_names, values,
                      /*origin_is_dml=*/true);
  mutation.SetLastOpFallbackDefaultValues(std::move(fallback_defaults));
...
bool IsPendingCommitTimestamp(const googlesql::ResolvedDMLValue& dml_value) {
  if (dml_value.value()->Is<googlesql::ResolvedFunctionCall>()) {
    const googlesql::ResolvedFunctionCall* fn =
        dml_value.value()->GetAs<googlesql::ResolvedFunctionCall>();
...
bool IsDefaultValue(const googlesql::ResolvedDMLValue& dml_value) {
  return dml_value.value()->Is<googlesql::ResolvedDMLDefault>();
}

absl::StatusOr<CaseInsensitiveStringSet> PendingCommitTimestampColumnsInInsert(
    const googlesql::Table* table,
    const std::vector<googlesql::ResolvedColumn>& insert_columns,
    const std::vector<std::unique_ptr<const googlesql::ResolvedInsertRow>>&
        insert_rows) {
  CaseInsensitiveStringSet default_pct_cols;
  for (int i = 0; i < table->NumColumns(); ++i) {
    const googlesql::Column* gsql_column = table->GetColumn(i);
...
absl::StatusOr<CaseInsensitiveStringSet> PendingCommitTimestampColumnsInUpdate(
  ...
  for (const auto& update_item : update_item_list) {
    if (!update_item->set_value() ||
        !update_item->target()->Is<googlesql::ResolvedColumnRef>()) {
      continue;
    }
...
absl::StatusOr<ExecuteUpdateResult> EvaluateResolvedInsert(
    const googlesql::ResolvedInsertStmt* insert_statement,
    const googlesql::ParameterValueMap& parameters,
    googlesql::TypeFactory* type_factory, DatabaseDialect database_dialect,
    const std::string time_zone,
    bool return_all_insert_rows_insert_ignore_dml = false) {
  if (insert_statement->insert_mode() ==
      googlesql::ResolvedInsertStmt::OR_REPLACE) {
    return error::UnsupportedUpsertQueries("Insert or replace");
  }
  if (return_all_insert_rows_insert_ignore_dml) {
    GOOGLESQL_RET_CHECK(insert_statement->insert_mode() ==
              googlesql::ResolvedInsertStmt::OR_IGNORE)
        << insert_statement->DebugString();
  }
  bool is_upsert_query = insert_statement->insert_mode() ==
                             googlesql::ResolvedInsertStmt::OR_IGNORE ||
                         insert_statement->insert_mode() ==
                             googlesql::ResolvedInsertStmt::OR_UPDATE;
  ...
  auto prepared_insert = std::make_unique<googlesql::PreparedModify>(
      insert_statement,
  ...
  GOOGLESQL_RETURN_IF_ERROR(prepared_insert->Prepare(analyzer_options));

  std::unique_ptr<googlesql::EvaluatorTableIterator> returning_iter;
  ...
  if (!status_or.ok()) {
    return MaybeTransformGoogleSQLDMLError(status_or.status());
  }
  ...
  if (std::get<2>(mutation_and_count) &&
    ...
    GOOGLESQL_RET_CHECK(insert_statement->insert_mode() ==
              googlesql::ResolvedInsertStmt::OR_IGNORE);
    GOOGLESQL_RET_CHECK(std::get<0>(mutation_and_count).ops().empty());
    GOOGLESQL_RET_CHECK_EQ(std::get<1>(mutation_and_count), 0);
  }
...
absl::StatusOr<ExecuteUpdateResult> EvaluateResolvedUpdate(
    const googlesql::ResolvedUpdateStmt* update_statement,
    const googlesql::ParameterValueMap& parameters,
    googlesql::TypeFactory* type_factory, const Schema* schema,
    const std::string time_zone) {
  GOOGLESQL_ASSIGN_OR_RETURN(auto updated_columns,
                   ColumnsInUpdate(update_statement->update_item_list()));

  auto prepared_update = std::make_unique<googlesql::PreparedModify>(
      update_statement, CommonEvaluatorOptions(type_factory, time_zone));
  GOOGLESQL_ASSIGN_OR_RETURN(auto analyzer_options,
                   MakeAnalyzerOptionsWithParameters(parameters, time_zone));
  GOOGLESQL_RETURN_IF_ERROR(prepared_update->Prepare(analyzer_options));
...
absl::StatusOr<ExecuteUpdateResult> EvaluateResolvedDelete(
    const googlesql::ResolvedDeleteStmt* delete_statement,
    const googlesql::ParameterValueMap& parameters,
    googlesql::TypeFactory* type_factory, const std::string time_zone) {
  auto prepared_delete = std::make_unique<googlesql::PreparedModify>(
      delete_statement, CommonEvaluatorOptions(type_factory, time_zone));
  GOOGLESQL_ASSIGN_OR_RETURN(auto analyzer_options,
                   MakeAnalyzerOptionsWithParameters(parameters, time_zone));
  GOOGLESQL_RETURN_IF_ERROR(prepared_delete->Prepare(analyzer_options));

  std::unique_ptr<googlesql::EvaluatorTableIterator> returning_iter;
  GOOGLESQL_ASSIGN_OR_RETURN(auto iterator,
                   prepared_delete->Execute(parameters, {}, &returning_iter));
  GOOGLESQL_ASSIGN_OR_RETURN(auto cursor,
...
absl::StatusOr<ExecuteUpdateResult> EvaluateUpdate(
    ...
    googlesql::Catalog* catalog, const googlesql::ParameterValueMap& parameters,
    googlesql::TypeFactory* type_factory, DatabaseDialect database_dialect,
  ...
  switch (resolved_statement->node_kind()) {
    ...
    case googlesql::RESOLVED_UPDATE_STMT:
      return EvaluateResolvedUpdate(
          resolved_statement->GetAs<googlesql::ResolvedUpdateStmt>(),
          parameters, type_factory, schema, time_zone);
    case googlesql::RESOLVED_DELETE_STMT:
      return EvaluateResolvedDelete(
          resolved_statement->GetAs<googlesql::ResolvedDeleteStmt>(),
          parameters, type_factory, time_zone);
    default:
      GOOGLESQL_RET_CHECK_FAIL() << "Unsupported support node kind "
...
absl::StatusOr<std::unique_ptr<RowCursor>> ResolveCallStatement() {
  ...
  types.push_back(googlesql::types::BoolType());
  return std::make_unique<VectorsRowCursor>(names, types, values);
}
...
absl::StatusOr<std::unique_ptr<RowCursor>> EvaluateQuery(
  ...
  if (query_mode == v1::ExecuteSqlRequest::PLAN) {
    ...
    GOOGLESQL_ASSIGN_OR_RETURN(auto iterator, prepared_query->Execute(params));
...
absl::StatusOr<std::map<std::string, googlesql::Value>> ExtractParameters(
    const Query& query, const googlesql::AnalyzerOutput* analyzer_output) {
  // Allow the loop below to look up undeclared parameters without worrying
  // about case. GoogleSQL will return the undeclared parameters using the
  ...
  for (const auto& [name, type] : analyzer_output->undeclared_parameters()) {
    ...
    // GoogleSQL will return an undeclared parameter error for any parameters
    // that do not have values specified (ie, when it == end()).
...
bool IsDMLStmt(const googlesql::ResolvedNodeKind& query_kind) {
  return query_kind == googlesql::RESOLVED_INSERT_STMT ||
         query_kind == googlesql::RESOLVED_UPDATE_STMT ||
         query_kind == googlesql::RESOLVED_DELETE_STMT;
}

bool IsDMLStmtWitoutSelect(
    const googlesql::ResolvedStatement* resolved_statement) {
  const googlesql::ResolvedNodeKind& query_kind =
      resolved_statement->node_kind();
  if (!IsDMLStmt(query_kind)) return false;

  if (query_kind == googlesql::RESOLVED_INSERT_STMT &&
      resolved_statement->GetAs<googlesql::ResolvedInsertStmt>()->query() !=
          nullptr) {
...
ExtractValidatedResolvedStatementAndOptions(
  ...
  GOOGLESQL_ASSIGN_OR_RETURN(
      statement,
      ann_functions_rewriter.ConsumeRootNode<googlesql::ResolvedStatement>());

  if (!ann_functions_rewriter.ann_functions().empty()) {
    ANNValidator ann_validator(context.schema);
    GOOGLESQL_RETURN_IF_ERROR(statement->Accept(&ann_validator));
  ...
  GOOGLESQL_RETURN_IF_ERROR(checker.CheckQueryAgainstLimits(statement.get()));
...
// statements modify.
class ExtractDmlTargetTableVisitor : public googlesql::ResolvedASTVisitor {
 public:
...
void NormalizeParameterNames(const Schema* schema, const Query& query,
                             Query* normalized_query) {
  *normalized_query = query;
  if (schema->dialect() == database_api::DatabaseDialect::POSTGRESQL) {
    for (const auto& [name, value] : query.declared_params) {
      if (std::all_of(name.begin(), name.end(), ::isdigit)) {
        normalized_query->declared_params["p" + name] = value;
        normalized_query->declared_params.erase(name);
      }
    }
    for (const auto& [name, value] : query.undeclared_params) {
      if (std::all_of(name.begin(), name.end(), ::isdigit)) {
        normalized_query->undeclared_params["p" + name] = value;
        normalized_query->undeclared_params.erase(name);
      }
    }
  }
}
...
absl::StatusOr<std::string> QueryEngine::GetDmlTargetTable(
  ...
  GOOGLESQL_ASSIGN_OR_RETURN(
      ...
      Analyze(normalized_query.sql, &catalog, analyzer_options, type_factory_));
  GOOGLESQL_ASSIGN_OR_RETURN(auto params,
...
absl::StatusOr<QueryResult> QueryEngine::ExecuteInsertOnConflictDml(
    const Query& query, const googlesql::ResolvedStatement* resolved_statement,
  ...
  if (on_conflict_clause->conflict_action() ==
          googlesql::ResolvedOnConflictClauseEnums::UPDATE &&
      context.schema->dialect() == database_api::DatabaseDialect::POSTGRESQL) {
    GOOGLESQL_ASSIGN_OR_RETURN(bool can_translate_to_insert_or_update_mode,
  ...
  GOOGLESQL_RETURN_IF_ERROR(ResolveConflictTarget(
  ...
  GOOGLESQL_RETURN_IF_ERROR(ExtractColumnInfoAndRowsForInsertOrUpdateAction(
  ...
  if (on_conflict_clause->conflict_action() ==
          googlesql::ResolvedOnConflictClauseEnums::UPDATE &&
      !existing_rows.empty()) {
    GOOGLESQL_ASSIGN_OR_RETURN(auto update_existing_rows_resolved_stmt,
...
const googlesql::ResolvedReturningClause* GetReturningClause(
    const googlesql::ResolvedStatement* resolved_statement) {
  const googlesql::ResolvedReturningClause* returning_clause;
  switch (resolved_statement->node_kind()) {
    case googlesql::RESOLVED_INSERT_STMT:
      returning_clause =
          resolved_statement->GetAs<googlesql::ResolvedInsertStmt>()
...
absl::Status QueryEngine::IsPartitionable(const Query& query,
                                          const QueryContext& context) const {
  Query local_query;
  NormalizeParameterNames(context.schema, query, &local_query);
  GOOGLESQL_ASSIGN_OR_RETURN(auto analyzer_options,
                   MakeAnalyzerOptionsWithParameters(
...
bool IsDMLQuery(const std::string& query) {
  googlesql::ResolvedNodeKind query_kind = googlesql::GetStatementKind(query);
...
QueryEngine::TryGetChangeStreamMetadata(const Query& query,
  ...
  GOOGLESQL_ASSIGN_OR_RETURN(auto analyzer_options,
                   MakeAnalyzerOptionsWithParameters(
                       local_query.declared_params, GetTimeZone(schema)));
  analyzer_options.set_prune_unused_columns(true);
  ...
  GOOGLESQL_ASSIGN_OR_RETURN(auto params,
                   ExtractParameters(local_query, analyzer_output.get()));
